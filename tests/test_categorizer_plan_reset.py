...
def test_service_plan_cleared_on_topic_change(mock_factory):
    _mock_category(mock_factory, MessageCategory.GREETING.value)
    out = message_categorizer_node(_state())
    assert out["service_plan"] is None


@patch("src.nodes.message_categorizer_node.message_categorizer_agent")
def test_service_plan_cleared_on_fresh_appointment_request(mock_factory):
    _mock_category(mock_factory, MessageCategory.BOOK_APPOINTMENT.value)
    out = message_categorizer_node(
        _state(active_flow=None, current_message="I want to book Friday")
    )
    assert out["service_plan"] is None
    assert out["active_flow"] == "booking"


@patch("src.nodes.message_categorizer_node.message_categorizer_agent")
def test_service_plan_kept_for_provided_information_during_booking_flow(mock_factory):
    _mock_category(mock_factory, MessageCategory.PROVIDE_INFORMATION.value)
